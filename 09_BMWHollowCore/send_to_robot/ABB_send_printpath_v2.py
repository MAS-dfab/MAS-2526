...
# NOTE: These are now defaults. Dynamic fan control will override based on overhang detection
COOLING_HC_FANS_1_ON = True
COOLING_HC_FANS_2_ON = True
COOLING_HC_AIR_ON = False
COOLING_HC_NOZZLE = False

# NEW: Dynamic fan control based on overhang detection from JSON
DYNAMIC_FAN_CONTROL = True  # Set to False to use static fan settings above
...
# NEW: First layer speed override (35% of base speed: 18 * 0.35 = 6.3, rounded up to 7)
FIRST_LAYER_SPEED_OVERRIDE = 7.0  # Set to None to disable first layer speed override
...
if dict_data:
    for i in range(len(dict_data)):
        ...
        # NEW: collect fan control flags from JSON
        fan1_vals.append(data_point.get("fan1_on", False))
        fan2_vals.append(data_point.get("fan2_on", False))
...
# ==============================================================================
# NEW: First layer speed override
# ==============================================================================
if FIRST_LAYER_SPEED_OVERRIDE is not None:
    first_layer_count = 0
    for i, layer_idx in enumerate(layer_indexes):
        if layer_idx == 0:
            velocities[i] = FIRST_LAYER_SPEED_OVERRIDE
            first_layer_count += 1
    print(f"First layer speed set to {FIRST_LAYER_SPEED_OVERRIDE} mm/s for {first_layer_count} points")
...
# NEW: split fan control lists
fan1_vals = fan1_vals[START_INDEX:STOP_INDEX]
fan2_vals = fan2_vals[START_INDEX:STOP_INDEX]
...
# NEW: Report overhang fan control stats
if DYNAMIC_FAN_CONTROL:
    fan_enabled_count = sum(1 for f1, f2 in zip(fan1_vals, fan2_vals) if f1 or f2)
    print(f"Dynamic fan control enabled: {fan_enabled_count} overhang points detected")
...
if not ROBOT_ON:
    ...
    if __name__ == "__main__":
        ...
        # NEW: Initialize fans based on mode
        if not DYNAMIC_FAN_CONTROL:
            # Static fan control (original behavior)
            if COOLING_HC_FANS_1_ON:
                abb.send(rrc.SetDigital(IO_C1_FAN1, 1))  # turn air on
                print("Fans 1 active (static)")

            if COOLING_HC_FANS_2_ON:
                abb.send(rrc.SetDigital(IO_C2_FAN2, 1))  # turn air on
                print("Fans 2 active (static)")
        else:
            # Dynamic fan control - start with fans off
            abb.send(rrc.SetDigital(IO_C1_FAN1, 0))
            abb.send(rrc.SetDigital(IO_C2_FAN2, 0))
            print("Dynamic fan control enabled - fans will activate at overhangs")
        ...
        # NEW: fan state tracking
        fan1_state = None
        fan2_state = None

        for i, (frame, v, z, ext_tg, hc_setpt, airp, li,
                trig0, trig1, motor_speed_analog, fan1, fan2) in enumerate(
            zip(
                moved_abb_print_frames,
                velocities,
                zones,
                ...
                trigger_motor_0_vals,
                trigger_motor_1_vals,
                motor_speeds_analog,     # ← analog input
                fan1_vals,               # ← NEW: fan control
                fan2_vals                # ← NEW: fan control
            )
        ):
            ...
            # NEW: ------- Dynamic fan control ----------------------------------------
            if DYNAMIC_FAN_CONTROL:
                if fan1 != fan1_state:
                    abb.send(rrc.SetDigital(IO_C1_FAN1, 1 if fan1 else 0))
                    fan1_state = fan1
                    if fan1:
                        print(f"Fan 1 activated at point {i} (overhang)")
                
                if fan2 != fan2_state:
                    abb.send(rrc.SetDigital(IO_C2_FAN2, 1 if fan2 else 0))
                    fan2_state = fan2
                    if fan2:
                        print(f"Fan 2 activated at point {i} (overhang)")
