...
class ModuleAggregation:
    ...
    def compute_next_module(self,
        ...
        # 1. 用上一組 module 作為基準
        # =========================================================
        base_module = self.my_modules[-1]   # important!!!

        # FROM frame
        from_stick = base_module[from_stick_index]
        from_frame = from_stick.eval_frame(face_index=from_face_index, 
                                        t_value=from_t, 
                                        z_offset=True)
        self.from_frame = from_frame

        # TO frame on the SAME base module
        ...
        # =========================================================
        # 3. Transform entire FIRST module → new module
        # =========================================================
        new_module = []
    ...
    def move_by_layer(self, translation_vector=(0, 0, 0), layer_iteration=1):
        ...
        for i in range(layer_iteration):
            T = Translation.from_vector(t_vec * i)

            translated_modules = []
            # transform ALL modules in my_modules
            for module in self.my_modules:
                new_module = []
                for stick in module:
                    new_stick = Stick(stick.frame.transformed(T), stick.length)
                    new_module.append(new_stick)
                translated_modules.append(new_module)

            # self.layer_modules.append(translated_modules)
            self.layer_modules.append(translated_modules)
