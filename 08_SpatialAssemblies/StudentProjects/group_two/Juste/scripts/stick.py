...
class Stick:
    ...
    # ------------------------------------------------------------
    # INTERNAL: Build local frame from axis + inherited parent frame
    # ------------------------------------------------------------

    def _build_frame_from_axis(self, axis, parent_frame=None):
        """Construct a Frame from the axis while preserving parent orientation."""
        ...
        # SAFE midpoint
        origin_pt = axis.point_at(0.5)
        origin = Vector(origin_pt.x, origin_pt.y, origin_pt.z)
    ...
    def intersects(self, other, clearance=0.0):
        """Very basic capsule-like collision test."""
