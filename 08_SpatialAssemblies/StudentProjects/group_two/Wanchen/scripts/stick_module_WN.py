...
class LoopModuleNoCross:
    """
    Deterministic closed-loop generator (方案 2).
    - 前 N-1 段长度固定
    - 相邻搭接处 start = prev_end - prev_dir * overlap_distance (可控)
    - 上下交替（z offset = ± depth/2）
    - 最后一段长度受限于 stick_length ± last_length_limit
    - 在 max_attempts 内随机尝试角序列与最后起点偏移，直到找到满足闭合容差的解
    """
    ...
    # ----------------------------
    def random_angle(self, prev_angle):
        # Always left turn — eliminates self intersections
        angle = random.uniform(self.min_turn, self.max_turn)

        if abs(angle - prev_angle) < self.min_distance:
            return angle
    ...
    def _generate_valid_loop(self):
        ...
        for attempt in range(self.max_attempts):
            ...
            # --------------------------
            pts = [root]           # ← use pts, NOT self.points
            direction = self.root_frame.xaxis.copy()
            prev_angle = 0
            ...
            for i in range(self.num_segments - 1):
                ...
                angle = self.random_angle(prev_angle)
                prev_angle = angle

                direction = direction.rotated(angle, self.root_frame.zaxis)

                # use pts[-1] instead of self.points[-1]
                next_pt = pts[-1] + direction * (self.stick_length - self.overlap_distance)

                # you may have own check functions here
                # if next_pt violates constraints → retry whole attempt
                # I keep your structure:
                # if self.too_close(next_pt) or self.exceeds_span(next_pt):
                #    break to next attempt
                # BUT be careful: you wrote "return False" in your code
                # which will incorrectly exit entire loop.
                # Here I use `break` instead so the next attempt runs.

                if hasattr(self, "too_close") and self.too_close(next_pt):
                    break
                if hasattr(self, "exceeds_span") and self.exceeds_span(next_pt):
                    break
                ...
                # Continue to closure logic

                prev_pt = pts[-1]
                prev_prev_pt = pts[-2]
                prev_dir_vec = Vector.from_start_end(prev_prev_pt, prev_pt)

                if prev_dir_vec.length == 0:
                    continue

                prev_unit = prev_dir_vec.unitized()

                samples = [0.0, -self.overlap_distance, self.overlap_distance,
                        -0.5 * self.overlap_distance, 0.5 * self.overlap_distance]

                for s in samples:

                    last_start = prev_pt + prev_unit * s
                    vec_to_root = Vector.from_start_end(last_start, root)
                    dist = vec_to_root.length
                    ...
                    if dist == 0:
                        ...
                        max_len = self.stick_length + self.last_length_limit
                        min_len = max(0.0, self.stick_length - self.last_length_limit)
                        L = max(min(dist, max_len), min_len)
                        final_end = last_start + vec_to_root.unitized() * L

                    if distance_point_point(final_end, root) <= self.closure_tol:

                        anchor_pts = pts[:-1] + [last_start, final_end]
                        ...
                        for i in range(len(anchor_pts) - 1):
                            ...
                            if i == 0:
                                ...
                                prev_a = anchor_pts[i - 1]
                                prev_dir = Vector.from_start_end(prev_a, a).unitized()
                                start_pt = a - prev_dir * self.overlap_distance

                            axes.append((start_pt, b))

                        if self._segments_too_close(axes):
                            continue

                        # success
                        self.points = anchor_pts
                        self._temp_axes = axes
                        return True
        return False
    # ----------------------------
    def _build_sticks_from_lines(self):
        ...
        for (s_pt, e_pt) in self._temp_axes:
            ...
            zdir = Vector(0, 0, 1) if up else Vector(0, 0, -1)
            offs = Vector(zdir.x * half, zdir.y * half, zdir.z * half)
            start_off = Point(s_pt.x + offs.x, s_pt.y + offs.y, s_pt.z + offs.z)
            end_off = Point(e_pt.x + offs.x, e_pt.y + offs.y, e_pt.z + offs.z)
            line = Line(start_off, end_off)
            self.lines.append(line)
            stick = Stick(line, z_vector=zdir, width=self.width, depth=self.depth)
            self.sticks.append(stick)
