...
class BranchStickModules:
    ...
    def grow_module_update(self,  offset_axis, offset_axis_b,
                from_module_index=-1, from_stick_index=-1,
                face_index=0, angle=0.0):
        """
        Grows a new module from an existing module's stick.

        Args:
            offset_axis: extra offset along the new module x axis
            offset_axis_b: offset along the new module y axis
            from_module_index: Index of module to grow from
            from_stick_index: Index of stick within that module to grow from 
            face_index: Index of the face to grow from (0-3)
        ...
        # 1. Get position on original stick from specific module
        position = self.get_face_frame(from_module_index,
                                    from_stick_index,
                                    face_index).copy()
        
        # 2. Geser dikit supaya koneksi di tengah face
        position.point += position.yaxis * (self.depth / 2.0 * offset_axis_b)
        position.point -= position.xaxis * (self.width / 2.0)

        # 3. ROTATE MODULE FRAME around local z axis
        Rz = Rotation.from_axis_and_angle(position.zaxis,
                                        math.radians(angle),
                                        point=position.point)
        position.transform(Rz)

        # 4. Geser keluar sepanjang x supaya modul ga tabrakan
        clearance = self.stick_length + self.width * 7.0
        position.point += position.xaxis * (clearance + offset_axis)

        # 5. Create new module at this position
        new_module = StickModuleC(position,
                                self.width,
                                self.depth,
                                self.stick_length)
        new_module.create_module_c()
        
        # 6. EXTRA ROTATION ONLY FOR THE Z-STICK
        #    (biar gak keliatan "ketinggalan")
        #    Ambil stick terakhir (stick_z)
        stick_z = new_module.sticks[-1]

        #    Bikin rotasi sekitar sumbu x lokal modul (frame.xaxis)
        Rx = Rotation.from_axis_and_angle(new_module.frame.xaxis,
                                        math.radians(angle),
                                        point=new_module.frame.point)

        #    Apply transform hanya ke geometry stick ini
        #    (pakai transform in-place, jangan assign .geometry = ...)
        stick_z.geometry.transform(Rx)

        # 7. Simpan modul ke list
        self.modules.append(new_module)
