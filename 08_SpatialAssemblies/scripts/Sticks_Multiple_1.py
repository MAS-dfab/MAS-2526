...
class TiltedModule:
    ...
    #     rotation_origin = self.root_frame.point + Vector(0,1,0) * m_distanece
    #     # rotation_axis = Line(rotation_origin, Point(rotation_origin.x, rotation_origin.y, rotation_origin.z + 10))
    #     rotation_axis = Line(rotation_origin, rotation_origin + Vector(0,0,1) * 10)
    #     R = Rotation.from_axis_and_angle(axis = rotation_axis, angle = math.radians(angle), point = rotation_origin)
    ...
    #     # Add rotated sticks to sticks list
    #     self.sticks.extend(rotated_sticks)

    # def mirror_stick(self, mirror_distance = 113):

    #     # Build mirror frame
    #     # mirror_origin = self.root_frame.point + self.root_frame.yaxis * mirror_distance
    #     mirror_origin = self.root_frame.point + Vector(0,1,0) * mirror_distance
    #     # mirror_normal = self.root_frame.yaxis
    #     # mirror_frame = Frame(point = mirror_origin, xaxis = self.root_frame.xaxis * -1, yaxis = self.root_frame.yaxis * -1)
    #     # mirror_frame = Frame(point = mirror_origin, xaxis = Vector(0,0,1), yaxis = Vector(0,-1,0))
    #     mirror_frame = Frame(point = mirror_origin, xaxis = self.root_frame.zaxis * 1, yaxis = self.root_frame.yaxis * -1)

    #     # mirror_frame.point -= mirror_frame.xaxis * self.depth

    #     # Transform frame
    #     M = Transformation.from_frame_to_frame(frame_from = self.root_frame, frame_to = mirror_frame)
    ...
    #         # Mirror axis（Line）
    #         new_axis = stick.axis.transformed(M)

    #         # Mirror frame（Frame）
    #         new_frame = stick.frame.transformed(M)
    #         # new_frame = mirror_frame.transformed(M)


    #         # Mirror z_vector !!
    #         # new_z = stick.z_vector.transformed(M)
    ...
    #         mirrored_sticks.append(new_stick)

    #     # Add mirrored sticks to sticks list
    #     self.sticks.extend(mirrored_sticks)
